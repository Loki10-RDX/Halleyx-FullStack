...
export default function WorkflowRulesPage() {
  ...
  if (loading) return <div className="p-8 text-center text-surface-400">Loading rules...</div>;
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-sm text-surface-500">
        ...
        <span className="text-surface-300">Manage Rules</span>
      </div>

      <div className="flex items-center justify-between">
        <div>
          <h1 className="page-title">Manage Rules</h1>
          <p className="mt-2 text-surface-400">Configure transition rules for <span className="text-white font-medium">{workflow?.name}</span></p>
        </div>
      </div>

      <RuleManager steps={steps} />
    </div>
  );
}
